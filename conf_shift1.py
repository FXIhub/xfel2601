...
def onEvent(evt):
    ...
    dets = np.concatenate(tuple([evt['photonPixelDetectors']['DSSC%.2d'%m].data for m in module_numbers]), axis=1)
    #print(dets.shape)
    det = evt['photonPixelDetectors']['DSSC00'].data
    #module = add_record(evt["analysis"], "analysis", "single", det[...,:n_pulses]-dark_cal[:,:,:n_pulses])
    module = add_record(evt["analysis"], "analysis", "single", dets[...,:n_pulses])
